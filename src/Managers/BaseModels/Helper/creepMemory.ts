...
export default class implements ICreepHelper {
    ...
    static GetMemory(id:string): DoubleCRUDResult<CreepMemory,CreepCache> {
        const result: DoubleCRUDResult<CreepMemory,CreepCache> = {success:false, memory:undefined, cache:undefined};
    ...
    static CreateMemory(id:string, memory:CreepMemory, cache:CreepCache): DoubleCRUDResult<CreepMemory,CreepCache> {
        const result: DoubleCRUDResult<CreepMemory,CreepCache> = {success:false, memory:undefined, cache:undefined};  
    ...
    static DeleteMemory(id:string, isMemory:boolean, isCache:boolean): DoubleCRUDResult<CreepMemory,CreepCache> {
        const result: DoubleCRUDResult<CreepMemory,CreepCache> = {success:false, memory:undefined, cache:undefined};
        if (isMemory) {
    ...
    static UpdateMemory(id:string, memory?:CreepMemory, cache?:CreepCache): DoubleCRUDResult<CreepMemory,CreepCache> {
        const result: DoubleCRUDResult<CreepMemory,CreepCache> = {success:false, memory:undefined, cache:undefined};
    ...
    static Initialize(data:CreepInitializationData): DoubleCRUDResult<CreepMemory,CreepCache> {
        ...
        const cacheResult = ICreepCache.Initialize(id,data.executer,data.body,data.pos,data.type);
        if (cacheResult.success) {
            result.success = true;
            result.cache = cacheResult.data;
        }
