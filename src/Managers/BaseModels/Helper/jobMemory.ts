...
export default class implements IJobHelper {
    ...
    static GetMemory(id:string): DoubleCRUDResult<JobMemory,JobCache> {
        const result: DoubleCRUDResult<JobMemory,JobCache> = {success:false, memory:undefined, cache:undefined};
    ...
    static CreateMemory(id:string, memory:JobMemory, cache:JobCache): DoubleCRUDResult<JobMemory,JobCache> {
        const result: DoubleCRUDResult<JobMemory,JobCache> = {success:false, memory:undefined, cache:undefined};
    ...
    static DeleteMemory(id:string, isMemory:boolean, isCache:boolean): DoubleCRUDResult<JobMemory,JobCache> {
        const result: DoubleCRUDResult<JobMemory,JobCache> = {success:false, memory:undefined, cache:undefined};
        if (isMemory) {
    ...
    static UpdateMemory(id:string, memory?:JobMemory, cache?:JobCache): DoubleCRUDResult<JobMemory,JobCache> {
        const result: DoubleCRUDResult<JobMemory,JobCache> = {success:false, memory:undefined, cache:undefined};
    ...
    static Initialize(data:JobInitializationData): DoubleCRUDResult<JobMemory,JobCache> {
        const id = this.GetJobId(data.type, data.pos);
        const result: DoubleCRUDResult<JobMemory,JobCache> = {success:false, memory:undefined, cache:undefined};
        const memoryResult = IJobMemory.Initialize(id,data.targetId,data.pos,data.amountToTransfer,data.fromTargetId);
        if (memoryResult.success) {
            result.success = true;
            result.memory = memoryResult.data;
        }
