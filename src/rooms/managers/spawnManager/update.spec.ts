...
describe("UpdateQueue", () => {
  ...
  it("Should_DoNothing_WhenNoCreepIsRequiredBecauseNoJobsInCach", () => {
    ...
    Memory.creepsData.data = {"creep":{creepType:"pioneer",lastExecutedAtTick:0,manager:{name:"spawn",roomName:room.name}},"creep2":{creepType:"carry",lastExecutedAtTick:0,manager:{name:"spawn",roomName:room.name},},"creep3":{creepType:"carry",lastExecutedAtTick:0,manager:{name:"spawn",roomName:room.name}}};
    Game.creeps = {"creep":mockInstanceOf<Creep>({body:[{type:WORK}]}),"creep2":mockInstanceOf<Creep>({body:[{type:WORK}]})};
  ...
  it("Should_DoNothing_WhenNoCreepIsRequiredBecauseQueueIsFull", () => {
    // Arrange
    const cache = Memory.roomsData.data[room.name].managersMemory.spawn;
    cache.queue = [mockInstanceOf<QueueCreep>({creepType:"pioneer", body: [WORK, CARRY, MOVE], managerName: "spawn"})];
  ...
  it("Should_AddAnCreepToTheQueueForEachCreepType_When_BodyPartsMissing",() => {
        // Arrange
        const cache = Memory.roomsData.data[room.name].managersMemory.spawn;
        cache.jobs = [JobCreatorHelper.HarvestMineral({amount:100000, id:"mineral",pos:new RoomPosition(0,0,room.name),type:"H"})];
  ...
  it("Should_NotAddBody_When_BodyIsEmpty", () => {
    // Arrange
    const cache = Memory.roomsData.data[room.name].managersMemory.spawn;
    const spyBodyFunction = jest.spyOn(BodyHelper, "Generate").mockReturnValueOnce([[]]);
